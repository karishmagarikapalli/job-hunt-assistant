...
# Updated test script for local clone and setup workflow
...
echo "Checking for database initialization script..."
...
# Test database initialization (without actually running it)
echo "Validating database initialization script..."
if grep -q "CREATE TABLE" "init_database.sh"; then
    echo "✅ Database initialization script contains table creation commands"
else
    echo "❌ Database initialization script does not contain table creation commands"
    exit 1
fi
